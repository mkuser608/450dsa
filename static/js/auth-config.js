      ...
      if (Alpine.store("auth")) {
        ...
        authStore.loginWithGoogle = async function () {
          ...
          try {
            ...
            await this.syncUserData();
            Alpine.store("status").setMessage(
          ...
          Object.keys(progress).forEach((topic) => {
            if (progress[topic] && Array.isArray(progress[topic])) {
              localStorage.setItem(
                `topic-${topic}`,
                JSON.stringify(progress[topic])
              );
            }
          });
          localStorage.setItem("localLastUpdated", Date.now()); // Track last local update
        };
        ...
        authStore.syncUserData = async function () {
          ...
          try {
            ...
            if (userDoc.exists) {
              ...
              // Get local progress and timestamp
              const localProgress = this.getLocalProgress();
              const localLastUpdated =
                localStorage.getItem("localLastUpdated") || 0;

              // Get cloud progress and timestamp
              const cloudProgress = userData.progress || {};
              const cloudLastSynced = userData.lastSynced?.toMillis() || 0;

              // Merge progress from both sources
              ...
              if (isSameAsLocal && isSameAsCloud) {
        ...
        authStore.mergeProgress = function (localProgress, cloudProgress) {
          ...
          allTopics.forEach((topic) => {
            ...
            const combinedProgress = [
              ...new Set([...localTopicProgress, ...cloudTopicProgress]),
            ];
